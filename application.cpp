...
void Application::RestoreQ2()
{
	...
	for(i=0; i< g_applic->_TotalFunctions; i++)
	{
		g_applic->_Functions[i].setExecContrib(0.0);
	}
	...
	while(mfile)
	{
		...
		g_applic->_Functions[fno].setExecContrib(contrib);
	}
	...
	for(i=0 ; i < g_applic->_TotalFunctions ; i++)
	{
		for(j=0;j< g_applic->_TotalFunctions;j++)
		{
			g_applic->_Edges[i][j].setWeight(0.0); 
		}
	}	
	...
	while(qfile)
	{
		...
		g_applic->_Edges[pfno][cfno].setWeight(bytes); 
		totalcomm += bytes;
		if(bytes > maxcomm) maxcomm = bytes;
	}
	
	g_applic->_MaxComm = maxcomm;
	g_applic->_TotalComm = totalcomm;
	...
	for(i=0 ; i < g_applic->_TotalFunctions ; i++)
	{
		for(j=0;j< g_applic->_TotalFunctions;j++)
		{
			g_applic->_Edges[i][j].setWeight( g_applic->_Edges[i][j].getWeight()/totalcomm * 100 ); 
		}
	}
